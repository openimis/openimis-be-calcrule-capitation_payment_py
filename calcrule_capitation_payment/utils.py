...
def claim_batch_valuation(payment_plan, work_data):
    ...
    if start_date is not None:
        ...
        capitation_index, distribution = get_contribution_index_rate(
            value, pp_params, work_data
        )
        # update the item and services
        items.update(
            price_valuated=F("price_adjusted") * capitation_index * distribution
        )
        services.update(
            price_valuated=F("price_adjusted") * capitation_index * distribution
        )
...
def generate_capitation(payment_plan, start_date, end_date, allocated_contribution):
    pp_params = obtain_calcrule_params(
        payment_plan, INTEGER_PARAMETERS, NONE_INTEGER_PARAMETERS
    )
    product = payment_plan.benefit_plan
    population_matter = (
        pp_params["weight_population"] > 0 or pp_params["weight_number_families"] > 0
    )
    year = end_date.year
    month = end_date.month
    if (
        pp_params["weight_insured_population"] > 0
        or pp_params["weight_number_insured_families"] > 0
        ...
        if pp_params["weight_number_insured_families"] > 0:
            sum_insured_families = get_product_sum_policies(
                product, start_date, end_date
            )
        # get the claim data
        sum_claim_adjusted_amount, sum_visits = 1, 1
        if (
            pp_params["weight_number_visits"] > 0
            or pp_params["weight_adjusted_amount"] > 0
        ):
            sum_claim_adjusted_amount, sum_visits = get_product_sum_claim(
                product, start_date, end_date, pp_params
            )

        # select HF concerned with capitation within the product location (new HF will come from claims)
        health_facilities = get_product_hf_filter(
            pp_params,
            get_capitation_health_facilites(product, pp_params, start_date, end_date),
        )
        health_facilities = health_facilities.prefetch_related(
            Prefetch(
                "location", queryset=Location.objects.filter(validity_to__isnull=True)
            )
        ).prefetch_related(
            Prefetch(
                "location__parent",
                queryset=Location.objects.filter(validity_to__isnull=True),
            )
        )
        ...
        for health_facility in health_facilities:
            ...
            generate_capitation_health_facility(
                ...
                sum_insurees,
                sum_insured_families,
                sum_pop,
                sum_families,
                sum_claim_adjusted_amount,
                sum_visits,
                year,
                month,
                start_date,
...
def get_product_hf_filter(pp_params, queryset):
    ...
    if (
        pp_params["hf_sublevel_1"] is not None
        or pp_params["hf_sublevel_2"] is not None
        or pp_params["hf_sublevel_3"] is not None
        or pp_params["hf_sublevel_4"] is not None
    ):
...
def generate_capitation_health_facility(
    ...
    sum_hf_insurees = 0
    if pp_params["weight_insured_population"] > 0:
        sum_hf_insurees = get_product_sum_insurees(
            product, start_date, end_date, health_facility
        )

    # get the sum of policy/insureed families
    sum_hf_insured_families = 0
    if pp_params["weight_number_insured_families"] > 0:
        sum_hf_insured_families = get_product_sum_policies(
            product, start_date, end_date, health_facility
        )

    sum_hf_claim_adjusted_amount, sum_hf_visits = 0, 0
    if pp_params["weight_number_visits"] > 0 or pp_params["weight_adjusted_amount"] > 0:
        sum_hf_claim_adjusted_amount, sum_hf_visits = get_product_sum_claim(
            product, start_date, end_date, pp_params, health_facility
        )
    ...
    alc_contri_ins_population = (
        allocated * pp_params["weight_insured_population"]
    ) / 100
    alc_contri_ins_families = (
        allocated * pp_params["weight_number_insured_families"]
    ) / 100
    ...
    up_ins_population = (
        alc_contri_ins_population / sum_insurees if sum_insurees > 0 else 0
    )
    up_ins_families = (
        alc_contri_ins_families / sum_insured_families
        if sum_insured_families > 0
        else 0
    )
    up_visits = alc_contri_visits / sum_visits if sum_visits > 0 else 0
    up_adjusted_amount = (
        decimal.Decimal(alc_contri_adjusted_amount) / sum_adjusted_amount
        if sum_adjusted_amount > 0
        else 0
    )
    ...
    if payment_cathment > 0:
        capitation = CapitationPayment(
            ...
            payment_cathment=total_population
            + total_families
            + total_ins_population
            + total_ins_families,
...
def get_product_villages(product):
    ...
    if districts is not None:
        villages = Location.objects.filter(validity_to__isnull=True).filter(
            parent__parent__in=districts
        )
    return villages
...
def get_capitation_health_facilites(product, pp_params, start_date, end_date):
    districts = get_product_districts(product)
    health_facilities_districts = (
        HealthFacility.objects.filter(validity_to__isnull=True)
    ...
    health_facilities_off_districts = (
        HealthFacility.objects.filter(validity_to__isnull=True)
        ...
        .filter(
            (
                Q(claim__items__product=product)
                & Q(claim__items__validity_to__isnull=True)
            )
            | (
                Q(claim__services__product=product)
                & Q(claim__services__validity_to__isnull=True)
            )
        )
    )
    ...
    if health_facilities_districts is not None:
        health_facilities = get_product_hf_filter(
            pp_params, health_facilities_districts | health_facilities_off_districts
        ).distinct()
        ...
        .annotate(
            sum_pop=Sum(
                (
                    ...
                    + Coalesce(F("other_population"), 0)
                )
                * F("catchments__catchment")
                / 100
            )
        )
        .annotate(
            sum_families=Sum(
                Coalesce(F("families"), 0) * F("catchments__catchment") / 100
            )
        )
    )
...
def get_product_sum_insurees(product, start_date, end_date, health_facility=None):
    ...
    if villages is not None:
        ...
        if health_facility is None:
            ...
            insurees = (
                insurees.filter(
                    policy__family__location__catchments__health_facility=health_facility
                )
                .filter(policy__family__location__catchments__validity_to__isnull=True)
                .annotate(
                    sum=Sum(F("policy__family__location__catchments__catchment"))
                    * Count("id")
                    / 100
                )
            )
...
def get_product_sum_policies(product, start_date, end_date, health_facility=None):
    ...
    if villages is not None:
        policies = (
            Policy.objects.filter(validity_to__isnull=True)
        ...
        if health_facility is None:
            ...
            policies = (
                policies.filter(
                    family__location__catchments__health_facility=health_facility
                )
                .filter(family__location__catchments__validity_to__isnull=True)
                .annotate(
                    sum=Sum(F("family__location__catchments__catchment"))
                    * Count("id")
                    / 100
                )
            )
...
def get_product_sum_population(product):
    villages = get_product_villages(product)
    if villages is not None:
        pop = villages.annotate(
            sum_pop=Sum(
                (F("male_population") + F("female_population") + F("other_population"))
            )
...
def get_product_sum_claim(
    product, start_date, end_date, pp_params, health_facility=None
):
    # make the items querysets
    items = (
        ClaimItem.objects.filter(validity_to__isnull=True)
        .filter(product=product)
    ...
    services = (
        ClaimService.objects.filter(validity_to__isnull=True)
        .filter(product=product)
        .filter(claim__process_stamp__lte=end_date)
        .filter(claim__process_stamp__gte=start_date)
    )
    ...
    if health_facility is not None:
        items = (
            items.filter(claim__health_facility=health_facility)
            .filter(get_hospital_level_filter(pp_params, prefix="claim__"))
            .filter(
                get_hospital_claim_filter(
                    product.ceiling_interpretation, pp_params["claim_type"], "claim__"
        ...
        services = (
            services.filter(claim__health_facility=health_facility)
            .filter(get_hospital_level_filter(pp_params, prefix="claim__"))
            .filter(
                get_hospital_claim_filter(
                    product.ceiling_interpretation, pp_params["claim_type"], "claim__"
    ...
    sum_visits = (
        items.values_list("claim_id", flat=True)
        .distinct()
        .union(services.values_list("claim_id", flat=True).distinct())
    ).count()
    ...
    if "sum" in value_services:
        sum_services += (
            value_services["sum"] if value_services["sum"] is not None else 0
        )
