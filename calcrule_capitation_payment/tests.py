...
class BatchRunWithCapitationPaymentTest(TestCase):
    ...
    def test_simple_batch(self):
        ...
        add_service_to_hf_pricelist(
            service, test_health_facility.id
        )
        ...
        item1 = create_test_claimitem(
            claim1,
            "A",
            custom_props={"price_asked": 100, "item_id": item.id, "qty_provided": 3},
        )
        ...
        self.assertEqual(
            claim1.remunerated, None, "Claim remunerated when it should not"
        )
        ...
        do_process_batch(self.user.id_for_audit, test_region.id, end_date)
        claim1.refresh_from_db()
        ...
        self.assertEquals(
            ...
            "Claim status should be valuated but it is not",
        )
